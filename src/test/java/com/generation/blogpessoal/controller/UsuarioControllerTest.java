...
public class UsuarioControllerTest {
	...
	public void deveCriarUmUsuario() {
		 ...
		 * Resposta da Requisição que será enviada pelo Objeto da Classe
		 * TestRestTemplate.
		 * 
		 * Na requisição HTTP será enviada a URL do recurso (/usuarios/cadastrar), o
		 * verbo (POST), a entidade HTTP criada acima (corpoRequisicao) e a Classe de
		 * retornos da Resposta (Usuario).
		 */
		ResponseEntity<Usuario> corpoResposta = testRestTemplate.exchange("/usuarios/cadastrar", HttpMethod.POST,
				corpoRequisicao, Usuario.class);

		/**
		 * Verifica se a requisição retornou o Status Code CREATED (201) Se for
		 * verdadeira, o teste passa, se não, o teste falha.
		 */
	...
	public void naoDeveDuplicarUsuario() {
		...
		/**
		 * Cria o mesmo objeto da Classe Usuario que foi persistido no Banco de dados na
		 * linha anterior para simular uma duplicação de usuário e insere dentro de um
		 * Objeto da Classe HttpEntity (Entidade HTTP)
		 */
		...
		/**
		 * Verifica se a requisição retornou o Status Code BAD_REQUEST (400), que indica
		 * que o usuário já existe no Banco de dados. Se for verdadeira, o teste passa,
		 * senão o teste falha.
		 */
	...
	public void deveAtualizarUmUsuario() {
		 ...
		 * TestRestTemplate.
		 * 
		 * Na requisição HTTP será enviada a URL do recurso (/usuarios/atualizar), o
		 * verbo (PUT), a entidade HTTP criada acima (corpoRequisicao) e a Classe de
		 * retornos da Resposta (Usuario).
		 * 
		 * Observe que o Método Atualizar não está liberado de autenticação (Login do
		 * usuário), por isso utilizamos o Método withBasicAuth para autenticar o
		 * usuário em memória, criado na BasicSecurityConfig.
		 * 
	...
	public void deveMostrarTodosUsuarios() {
		 ...
		 * TestRestTemplate.
		 * 
		 * Na requisição HTTP será enviada a URL do recurso (/usuarios/all), o verbo
		 * (GET), a entidade HTTP será nula (Requisição GET não envia nada no Corpo da
		 * Requisição) e a Classe de retorno da Resposta (String), porquê a lista de
		 * Usuários será do tipo String.
		 * 
		 * Observe que o Método All não está liberado de autenticação (Login do
		 * usuário), por isso utilizamos o Método withBasicAuth para autenticar o
		 * usuário em memória, criado na BasicSecurityConfig.
		 * 
		 * Usuário: root Senha: root
	...
	public void deveListarApenasUmUsuario() {
		 ...
		 * TestRestTemplate.
		 * 
		 * Na requisição HTTP será enviada a URL do recurso ("/usuarios/" +
		 * usuarioBusca.get().getId()), o verbo (GET), a entidade HTTP será nula
		 * (Requisição GET não envia nada no Corpo da Requisição) e a Classe de retorno
		 * da Resposta (String), porquê a lista de Usuários será do tipo String.
		 * 
		 * Para obtero Id de forma automática, foi utilizado o método getId() do Objeto
		 * usuarioBusca.
		 * 
		 * Observe que o Método All não está liberado de autenticação (Login do
		 * usuário), por isso utilizamos o Método withBasicAuth para autenticar o
	...
	public void deveAutenticarUsuario() {
		...
		/**
		 * Persiste um objeto da Classe Usuario no Banco de dados através do Método
		 * cadastrarUsuario da Classe UsuarioService
		 */
		usuarioService.cadastrarUsuario(new Usuario(0L, "Marisa Souza", "marisa_souza@email.com.br", "13465278",
				"https://i.imgur.com/T12NIp9.jpg"));

		/**
		 * Cria um Objeto da Classe UsuarioLogin dentro de um Objeto da Classe
		 * HttpEntity (Entidade HTTP). O Objeto desta Classe será preenchido apenas como
		 ...
		 * TestRestTemplate.
		 * 
		 * Na requisição HTTP será enviada a URL do recurso (/usuarios/logar), o verbo
		 * (POST), a entidade HTTP criada acima (corpoRequisicao) e a Classe de retornos
		 * da Resposta (UsuarioLogin).
		 */
